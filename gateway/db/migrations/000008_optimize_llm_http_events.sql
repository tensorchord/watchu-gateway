-- Performance fix: optimize populate_llm_http_events to avoid full-table scan
--
-- Root cause: The original function used the response_lineage VIEW which forced
-- PostgreSQL to scan the entire http_response table (200K+ rows) before applying
-- time window filters (p_since/p_until). The VIEW's CTE structure and
-- row_number() window function prevented WHERE predicate pushdown.
-- This caused BufFileWrite IO waits (disk spill) and 30+ minute query times,
-- blocking the analysis scheduler and preventing watermark advancement.
--
-- Fix: Inline the response_lineage view logic with early time-window filtering
-- applied directly to http_response BEFORE any joins. Also adds missing indexes.
--
-- Additional Go fix in scheduler.go: Added 2-minute context timeout to
-- runOptionalStep to prevent slow enrichment steps from blocking the scheduler.

-- Create missing indexes for the response_lineage join pattern
CREATE INDEX IF NOT EXISTS idx_http_request_host_pid_ts
    ON http_request (host, pid, timestamp);
CREATE INDEX IF NOT EXISTS idx_http_response_host_pid_ts
    ON http_response (host, pid, timestamp);

-- Optimized populate_llm_http_events: inline response_lineage view with early filtering
...
BEGIN
    WITH windowed_response AS (
        -- First: narrow http_response to our time window BEFORE any joins
        SELECT
            r.host,
            r.id AS response_id,
    ...
    response_enriched AS (
        -- Join with process_lifecycle using the already-filtered responses
        SELECT
            wr.host,
            wr.response_id,
            wr.response_ts,
    ...
    src AS (
        ...
        LEFT JOIN LATERAL (
            ...
            WHERE req.host = re.host
              AND req.pid = re.pid
              AND req.timestamp <= re.response_ts
