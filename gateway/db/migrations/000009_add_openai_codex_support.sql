-- Add OpenAI/Codex API support to populate_llm_http_events
--
-- Root cause: The populate_llm_http_events function only supports 'gemini' and 'claude-code'
-- providers. OpenClaw uses OpenAI-compatible Codex API (/backend-api/codex/responses) which
-- was not being parsed into llm_http_event table, resulting in 0 records for PI detection.
--
-- Fix: Extend the provider detection logic to recognize OpenAI/Codex API format:
-- - Request: {"model": "gpt-5.3-codex", "messages": [...], "stream": true}
-- - Response: SSE stream (but final accumulated response in response table)
-- - Provider detection: Check for "messages" key and OpenAI-style model naming
    ...
    normalized AS (
        SELECT
            ...
            CASE
                ...
                -- Codex: Check request for model name matching OpenAI patterns
                -- Codex uses simplified format with just 'model' and 'text' fields
                WHEN s.req_json->>'model' ~ '^(gpt-|codex-|o1-|o3-)' 
                THEN 'codex'
                -- Fallback: Check response for OpenAI format
    ...
    prepared AS (
        SELECT
            ...
            CASE
                ...
                -- For Codex SSE streams without complete response, use request hash
            ...
            CASE
                ...
                -- For Codex, check for messages or text field
        ...
        WHERE (
            ...
            -- For Codex, accept records with valid model in request
